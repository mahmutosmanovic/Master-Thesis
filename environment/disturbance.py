...
DEFAULT_PROFILE = {
    "horizontal_points": [(20.0, 1.0), (50.0, 0.8), (110.0, 0.1), (500.0, 0.0)],
    "altitude_points": [(20.0, 1.0), (40.0, 0.4), (110.0, 0.1), (500.0, 0.0)],
    "angle_weight": 1.0,
    "base_weight": 1.0,
}

def disturbance_gain_alt(dist_vec, profile=None, field_boost=0.0):
    if profile is None:
        profile = DEFAULT_PROFILE

    g_h = horizontal_gain(
        dist_vec,
        points=profile["horizontal_points"],
        field_boost=field_boost,
    )
    g_v = altitude_gain(
        dist_vec,
        points=profile["altitude_points"],
        field_boost=field_boost,
    )
    g_a = angle_gain(dist_vec)

    base = profile["base_weight"] * (g_h * g_v)

    comps = [
        profile["angle_weight"] * g_a
    ]

    den = profile["base_weight"] + profile["angle_weight"] + 1e-8
    D = (base + sum(comps)) / den
...
def altitude_gain(dist_vec, points=None, field_boost=0.0):
    if points is None:
        points = DEFAULT_PROFILE["altitude_points"]

    _, _, z = dist_vec
    z = abs(z)
    return piecewise_gain(z, points, field_boost=field_boost)


def horizontal_gain(dist_vec, points=None, field_boost=0.0):
    if points is None:
        points = DEFAULT_PROFILE["horizontal_points"]

    dx, dy, _ = dist_vec
    d = np.sqrt(dx * dx + dy * dy)
    return piecewise_gain(d, points, field_boost=field_boost)


def plot_gains(profile=None, field_boost=0.0):
    if profile is None:
        profile = DEFAULT_PROFILE
...
def distance_plot(profile=None, field_boost=0.0):
    if profile is None:
        profile = DEFAULT_PROFILE
...
def angle_distance_plot(profile=None, field_boost=0.0):
    if profile is None:
        profile = DEFAULT_PROFILE
...
def component_plots(profile=None, field_boost=0.0):
    if profile is None:
        profile = DEFAULT_PROFILE
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            ...
            G_h[i, j] = horizontal_gain(
                dist_vec,
                points=profile["horizontal_points"],
                field_boost=field_boost,
            )
            G_v[i, j] = altitude_gain(
                dist_vec,
                points=profile["altitude_points"],
                field_boost=field_boost,
            )
