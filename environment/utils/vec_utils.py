...
def position_in_view(target, pitch_rad, yaw_rad, distance):
    vec = pitched_vector(pitch_rad, yaw_rad)
    pos = target - (vec*distance)
    return pos

def pitched_vector(pitch_rad, yaw_rad):
    return np.array([ # Used by camera, drone facing direction
