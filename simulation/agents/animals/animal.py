...
class Animal(Agent):
    ...
    def random_policy(self, obs):
        ...
        angular_velocity = np.random.normal(0, self.turn_noise)
        accel = np.random.normal(0.5, 0.5)
...
class Eagle(Animal):
    def __init__(self, pos, mode="random"):
        ...
        self.max_turn  = 2.0       # rad/s (smooth, wide turns)
        self.max_accel = 8.0       # m/s^2 (powerful takeoff)

        # perception & cognition
        self.vision = 250.0        # m     (long-range vision)
        self.turn_noise = 0.05     # rad/s (very stable heading)
...
class Jackal(Animal):
    def __init__(self, pos, mode="random"):
        ...
        self.max_turn  = 1.5       # rad/s (body inertia)
        self.max_accel = 4.0       # m/s^2 (ground acceleration)

        # perception & cognition 
        self.vision = 100.0        # m     (horizontal vision)
        self.turn_noise = 0.15     # rad/s (moderate variability)
...
class Pigeon(Animal):
    def __init__(self, pos):
        ...
        self.max_turn  = 5.0       # rad/s (high maneuverability)
        self.max_accel = 6.0       # m/s^2 (quick bursts)

        # perception & cognition
        self.vision = 80.0         # m     (local perception)
        self.turn_noise = 0.25     # rad/s (exploratory jitter)
